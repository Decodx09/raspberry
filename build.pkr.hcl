...
source "qemu" "ubuntu-arm" {
  ...
  iso_checksum = "none" // Bypasses the download error
  ...
  qemuargs = [
    ...
    ["-smp", "2"],
    ["-m", "2048"],
    # Critical fix for running in a non-graphical environment
    ["-nographic"] 
  ]
...
build {
  ...
  # Run the final setup commands inside the image after OS installation
  provisioner "shell" {
    inline = [
      ...
      "sudo git clone https://github.com/google-gemini/simple-python-app.git /opt/app/blue", # Replace with your app's URL
